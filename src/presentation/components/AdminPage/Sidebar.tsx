...
const Sidebar: React.FC = () => {
  return (
    <div className="bg-gray-100 w-[302px] hidden h-screen lg:flex flex-col justify-between shadow-lg">
      ...
      <nav className="flex-1">
        <ul className="space-y-4 p-4">
          ...
          <li key={index} className="flex items-center space-x-3  text-blue-700 hover:bg-gray-200  rounded-md cursor-pointer">
              <a className=' p-2 w-full h-full' href={`/admin/${item.label.toLocaleLowerCase().replace(/\s+/g, "")}`}>  <span className="text-xl">{item.icon}</span>
              <span className="font-medium">{item.label}</span></a>
            </li>
          ))}
        </ul>
      </nav>
